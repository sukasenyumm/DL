...
namespace LibDL
{
    ...
    public class NeuralNetwork : INeuralNetwork
    {
        ...
        protected StochasticNetworkLayer[] slayers;

        public StochasticNetworkLayer[] sLayers
        {
            get { return slayers; }
        }
        ...
        public NeuralNetwork(bool useStochasticNetworkLayer,IActivationFunction function, int inputsCount, params int[] neuronsCount)
        {
            ...
            // create each layer
            if(useStochasticNetworkLayer)
            {
                // create collection of layers
                this.layers = this.slayers = new StochasticNetworkLayer[this.layersCount];
                for (int i = 0; i < slayers.Length; i++)
                {
                    layers[i] = slayers[i] = new StochasticNetworkLayer(
            ...
            {
                //// create collection of layers (KODE 1-1)
        ...
        public virtual double[] Compute(double[] input, bool useStochasticNetworkLayer = false)
        {
            // variabel lokal mencegah konfilk mutlithreading
            double[] output = input;

            // hitung output setiap layer
            if (useStochasticNetworkLayer)
            {
                for (int i = 0; i < slayers.Length; i++)
                {
                    output = slayers[i].Compute(output);
                }
